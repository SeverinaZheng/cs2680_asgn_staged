...
You will build `myagent.py`, a minimal coding-agent harness, and test it with SWE-bench Pro. An agent is a model in a loop. It *proposes* tool calls, and your harness
...
# Put your key here, directly in the script. Do NOT read it from an environment
# variable: your agent runs inside the task container, and anything you export on
# the host does not reach it unless it is passed through explicitly.
CS2680_API_KEY = "hyi-..."          # your key
...
resp = client.chat.completions.create(
    ...
    tools=[...],          # your tool definitions, see 1.1
)
```

Keep the key in one place — a single constant at the top of your config module, so
there is exactly one line to change.

In this assignment, GPT-5 mini will serve as the backbone model and should be used for
all evaluations.
...
```bash
python myagent.py -p "<problem statement>" [--log] <workdir>
```
...
- `--log` — trace logging, off by default. When set, your agent writes its run
  trace into a `myagent_logs/` directory in the current working directory (create
...
`myagent`**. You are encouraged to use an AI coding assistant for this assignment. To provide insight into your development process, please include logs of your AI-assisted sessions with your submission. The logs should be in `.jsonl` format.
...
### 0.3 Agent logs (`myagent_logs/`)

When run with `--log`, your agent should write its trace into `./myagent_logs/` as
JSONL (one file per run, e.g. `myagent_logs/run1.jsonl`): one JSON object per line,
...
```json
{"timestamp": "2026-09-12T14:03:07+00:00", "event": "run_start", "model_id": "gpt-5-mini", "workdir": "/app"}
...
```bash
#!/bin/bash
# docker_env.sh — container setup, runs before myagent.py
...
docker run --rm \
  ...
  -v "$PWD:/myagent" \
  "$IMG" \
  -c 'bash /myagent/docker_env.sh && \
      cd /tmp && \
      python /myagent/myagent.py -p "$(cat /myagent/problem.txt)" --log /app && \
      cp -r myagent_logs /myagent/ && \
      git -C /app diff' \
  > "model_patch_$IID.diff"
```

The script above identifies the ID and problem statement for each task, mounts the corresponding codebase in a container, and sets up the environment. It then runs myagent. After the agent finishes modifying the repository, the script exports the myagent logs and generates a diff patch.
...
| `run_task.sh <idx>` | pulls the task image and runs your agent in its container (2.1–2.3); writes `model_patch_<instance_id>.diff` and copies `myagent_logs/` back |
...
Run them from your repo root (the directory containing `myagent.py`); if you keep the
folder elsewhere, set `MYAGENT_REPO` to your repo root. The task JSONs are
always read from inside `evaluation_scripts/` itself, and all outputs (patches,
`predictions.json`, `myagent_logs/`, `pro_eval/`) land in the repo root:
...
requirement — if `myagent.py` is not at the repo root, our runner cannot call it and
...
```
<repo root>
├── myagent.py                              # REQUIRED: the entry point, at the ROOT
├── docker_env.sh                           # REQUIRED: container setup (see 2.2), at the ROOT
├── src/                                    # optional: the modules myagent.py imports
│   ├── agentic_loop.py                     #   (any structure you like — this is one example)
│   ├── tools.py
│   └── prompts.py
├── developer_logs/                         # how you built myagent (see 0.2)
